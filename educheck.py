...
class Server:
    ...
    def appendUserToExistingUsersList(self, user):
        if str(user[0]) in admins:
            existingUsers.append(Admin(mentionID=user[0], privacyPolicyIsAccepted=user[1], userIsLogged=user[2], userAuthData=user[3]))
        else:
            existingUsers.append(User(mentionID=user[0], privacyPolicyIsAccepted=user[1], userIsLogged=user[2], userAuthData=user[3]))
...
class User:
    ...
    def callAvailableRequests(self, request):
        availableRequests = {"Начать": self.sendIntroductionMessage,
                             "Принимаю": self.agreePrivacyPolicy,
                             "Войти": self.sendAuthInfoMessage,
                             "Табель успеваемости": (self.parseReportCard, "https://edu.tatar.ru/user/diary/term"),
        ...
        if self.testMode is False:
            if (time.time() - self.startTime > 2 and self.ignoreMode is True) or self.ignoreMode is False:
                if request == "Табель успеваемости" or request == "На сегодня" or request == "На завтра" or request == "На вчера":
                ...
                if self.wg is True:
                    vk.messages.send(peer_id=self.mentionID, message="Ты слишком часто отправляешь команды. Я буду игнорировать тебя в течение 5 секунд.", random_id=random.getrandbits(32))
                    self.wg = False
        else:
            if self.hg is True:
    ...
    def sendAuthInfoMessage(self):
        if (self.privacyPolicyIsAccepted is True or self.privacyPolicyIsAccepted == '1') and (self.userIsLogged == '0' or self.userIsLogged is False):
            vk.messages.send(peer_id=self.mentionID, message="Введи логин и пароль через пробел. \n Пример: 3010034456 QMYK", random_id=random.getrandbits(32))
            self.getUserAuthDataMode = True
        elif (self.privacyPolicyIsAccepted is True or self.privacyPolicyIsAccepted == '1') and (self.userIsLogged == '1' or self.userIsLogged is True):
            vk.messages.send(peer_id=self.mentionID, message="Ты уже вошёл в свой аккаунт. Хотите выйти? Нажмите на кнопку 'Выйти'.", random_id=random.getrandbits(32))
        elif self.privacyPolicyIsAccepted == '0' or self.privacyPolicyIsAccepted is False:
            vk.messages.send(peer_id=self.mentionID, message="Ты ещё не принял условия Политики конфиденциальности.", random_id=random.getrandbits(32))
            self.sendPrivacyPolicyMessage()

    def sendPrivacyPolicyMessage(self):
        vk.messages.send(peer_id=self.mentionID, message="Нажми на кнопку 'Принимаю' или напиши сам, чтобы принять условия Политики конфиденциальности", random_id=random.getrandbits(32), keyboard=acceptPrivacyPolicyKeyboard.get_keyboard())

    def sendIntroductionMessage(self):
        if self.privacyPolicyIsAccepted is False or self.privacyPolicyIsAccepted == '0':
            vk.messages.send(peer_id=self.mentionID, message="Привет! Мы ценим конфиденциальность пользовательских данных, поэтому предалагаем тебе прочитать Политику конфиденциальности (vk.com/@educheck-privacy-policy) нашего бота и принять её условия.", random_id=random.getrandbits(32))
            self.sendPrivacyPolicyMessage()
        else:
            if self.userIsLogged is True or self.userIsLogged == "1":
                vk.messages.send(peer_id=self.mentionID, message="Ты уже можешь пользоваться мною (открой клавиатуру)", random_id=random.getrandbits(32), keyboard=self.schoolCardsKeyboard.get_keyboard())
            if self.userIsLogged is False or self.userIsLogged == "0":
                vk.messages.send(peer_id=self.mentionID, message="Тебе осталось всего лишь войти!", random_id=random.getrandbits(32), keyboard=authKeyboard.get_keyboard())
    ...
    def agreePrivacyPolicy(self):
        if self.privacyPolicyIsAccepted is True or self.privacyPolicyIsAccepted == '1':
            vk.messages.send(peer_id=self.mentionID, message="Не надо. Ты уже принял Политику конфиденциальности нашего бота", random_id=random.getrandbits(32))
        else:
    ...
    def addNewUserData(self, connect, cursor):
        cursor.execute(f"""INSERT OR IGNORE INTO users(id, privacyPolicyIsAccepted, userIsLogged) VALUES({self.mentionID}, {'False'}, {'False'})""")
        connect.commit()
    ...
    def selectDay(self):
        vk.messages.send(peer_id=self.mentionID, message="Выбери подходящий тебе день:", random_id=random.getrandbits(32), keyboard=self.selectDayKeyboard.get_keyboard())

    def backToMain(self):
        vk.messages.send(peer_id=self.mentionID, message="Ты вернулся назад.", random_id=random.getrandbits(32), keyboard=self.schoolCardsKeyboard.get_keyboard())
    ...
    def parseDay(self, URL):
        ...
        self.reportCard[time.strftime("%a, %d %b %Y", time.localtime(int(URL.split("=")[1])))] = []
        for tag in resultTags:
            if len(tag.split("—")) == 2 and tag.count(":") == 2:
                p.append(tag)
                self.reportCard[time.strftime("%a, %d %b %Y", time.localtime(int(URL.split("=")[1])))].append(list(reversed(p)))
                p = []
            else:
                if len(tag) != 0:
                    tag = tag.strip()
        ...
        if len(list(self.reportCard.values())[0]) == 0:
            vk.messages.send(peer_id=self.mentionID, message="Ой, мы ничего не смогли найти... \n Возможно, сегодня или завтра выходной день? \n \n Если это не так, напиши, пожалуйста, в техническую поддержку.", random_id=random.getrandbits(32))    
        else:
            resultString += list(self.reportCard.keys())[0] + "\n \n"
            for index, subject in enumerate(list(reversed(list(self.reportCard.values())[0])), 1):
                if len(subject) == 6:
                    resultString += f"{index}. Время: {subject[0]} \n Предмет: {subject[1]} \n Что задали: {subject[2]} \n Комментарий: {subject[3]} \n Оценка: {', '.join([subject[4], subject[5]])} \n \n"
                else:
                    resultString += f"{index}. Время: {subject[0]} \n Предмет: {subject[1]} \n Что задали: {subject[2]} \n Комментарий: {subject[3]} \n Оценка: {subject[4]} \n \n"
            vk.messages.send(peer_id=self.mentionID, message=resultString, random_id=random.getrandbits(32))
    ...
    def returnContent(self):
        try:
            ...
            vk.messages.send(peer_id=self.mentionID, message="Не удалось получить необходимые данные. Попробуй ещё раз.", random_id=random.getrandbits(32))
...
class Admin(User):
    def __init__(self, mentionID, privacyPolicyIsAccepted=False, userIsLogged=False, getUserAuthDataMode=False, userAuthData=None, testMode=False, ignoreMode=False):
        ...
        self.adminKeyboard.add_button("Включить тестовый режим", color=VkKeyboardColor.PRIMARY)
    ...
    def callAvailableRequests(self, request):
        ...
        if self.testMode is False:
            if (time.time() - self.startTime > 5 and self.ignoreMode is True) or self.ignoreMode is False:
                if request == "Табель успеваемости" or request == "На сегодня" or request == "На завтра" or request == "На вчера":
                    if self.userIsLogged is True or self.userIsLogged == "1":
                        self.checkSessionIsValid()
                        availableRequests[request][0](availableRequests[request][1])
    ...
    def getAdminKeyboard(self):
        vk.messages.send(peer_id=self.mentionID, message="Клавиатура администратора:", random_id=random.getrandbits(32), keyboard=self.adminKeyboard.get_keyboard())

    def deactivateTestMode(self):
        for user in existingUsers:
            user.testMode = False
        vk.messages.send(peer_id=self.mentionID, message="Тестовый режим отключен для всех зарегистрированных пользователей.", random_id=random.getrandbits(32), keyboard=self.adminKeyboard.get_keyboard())
    ...
    def activateTestMode(self):
        for user in existingUsers:
            if str(user.mentionID) not in admins:
                user.testMode = True
                user.hg = True
        vk.messages.send(peer_id=self.mentionID, message="Тестовый режим включен для всех зарегистрированных пользователей.", random_id=random.getrandbits(32), keyboard=self.adminKeyboard.get_keyboard())
    ...
    def sendNotififactionMessage(self):
        for user in existingUsers:
            time.sleep(5)
            vk.messages.send(peer_id=user.mentionID, message="Мы вышли из режима тестирования, теперь ты можешь пользоваться ботом!", random_id=random.getrandbits(32))
    
server = Server(token="855c82e1ca471b9af66fdb369be1e59d6d8233f93d361126c4eebf228d44dc7f4d4169498b062b90c9fa9", groupID="188029668", databaseName="usersDB.db")
...
def eventHandler(event):
    try:
        if event.type == VkBotEventType.GROUP_JOIN:
            vk.messages.send(peer_id=event.obj["user_id"], message=f"Привет, {vk.users.get(user_ids=event.obj['user_id'])[0]['first_name']} \n Мы рады, что ты подписался на нас. \n \n Нажми на кнопку 'Начать', чтобы начать пользоваться нашим ботом.", random_id=random.getrandbits(32), keyboard=getStartedKeyboard.get_keyboard())
